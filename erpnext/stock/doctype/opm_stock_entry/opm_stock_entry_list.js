frappe.listview_settings['OPM Stock Entry'] = {
	add_fields: ["`tabOPM Stock Entry`.`from_warehouse`", "`tabOPM Stock Entry`.`to_warehouse`",
		"`tabOPM Stock Entry`.`purpose`", "`tabOPM Stock Entry`.`production_order`", "`tabOPM Stock Entry`.`bom_no`"],
	column_render: {
		"from_warehouse": function(doc) {
			var html = "";
	 		if(doc.from_warehouse) {
				html += '<span class="filterable h6"\
					data-filter="from_warehouse,=,'+doc.from_warehouse+'">'+doc.from_warehouse+' </span>';
			}
			if(doc.from_warehouse || doc.to_warehouse) {
				html += '<i class="octicon octfa fa-arrow-right text-muted"></i> ';
			}
			if(doc.to_warehouse) {
				html += '<span class="filterable h6"\
				data-filter="to_warehouse,=,'+doc.to_warehouse+'">'+doc.to_warehouse+'</span>';
			}
			return html;
		}
	}
};
