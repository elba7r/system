...
class OPMStockEntry(StockController):
	def get_feed(self):
		return _("From {0} to {1}").format(self.to_warehouse, self.from_warehouse)
	...
	def validate_warehouse(self):
		...
		for d in self.get('items'):
			if not d.s_warehouse and not d.t_warehouse:
				d.s_warehouse = self.to_warehouse
				d.t_warehouse = self.from_warehouse
			...
			if self.purpose in ["Manufacture", "Repack"]:
				if validate_for_manufacture_repack:
					if d.bom_no:
						...
						elif self.pro_doc and (cstr(d.s_warehouse) != self.pro_doc.fg_warehouse and cstr(d.s_warehouse) != self.pro_doc.scrap_warehouse):
							frappe.throw(_("Source warehouse in row {0} must be same as Production Order").format(d.idx))
	...
	def check_duplicate_entry_for_production_order(self):
		other_ste = [t[0] for t in frappe.db.get_values("OPM Stock Entry",  {
		...
		if other_ste:
			...
			fg_qty_already_entered = frappe.db.sql("""select sum(transfer_qty)
				from `tabOPM Stock Entry Detail`
				where parent in (%s)
					and item_code = %s
	...
	def set_serial_nos(self, production_order):
		previous_se = frappe.db.get_value("OPM Stock Entry", {"production_order": production_order,
				"purpose": "Material Transfer for Manufacture"}, "name")

		for d in self.get('items'):
			transferred_serial_no = frappe.db.get_value("OPM Stock Entry Detail",{"parent": previous_se,
		...
		if self.purpose == "Subcontract" and self.purchase_order:
			...
			for se_item in self.items:
				...
				total_supplied = frappe.db.sql("""select sum(qty)
					from `tabOPM Stock Entry Detail`, `tabOPM Stock Entry`
					where `tabOPM Stock Entry`.purchase_order = %s
						and `tabOPM Stock Entry`.docstatus = 1
						and `tabOPM Stock Entry Detail`.item_code = %s
						and `tabOPM Stock Entry Detail`.parent = `tabOPM Stock Entry`.name""",
	...
	def update_stock_ledger(self):
		...
		#			"incoming_rate": flt(flt(flt(flt(flt(self.total_outgoing_value) - flt(self.total_incoming_value)) / flt(self.total_incoming_value))*flt(d.valuation_rate))+flt(d.valuation_rate))
		#		}))
		        	if d.keep_valuation == 1:
 			        	sl_entries.append(self.get_sl_entries(d, {
				        	"warehouse": cstr(d.t_warehouse),
				    	    "actual_qty": flt(d.transfer_qty),
				    	    "incoming_rate": flt(d.valuation_rate)			
	...
	def get_gl_entries(self, warehouse_account):
		...
		gl_entries = super(OPMStockEntry, self).get_gl_entries(warehouse_account)
		...
		if self.bom_no:
			if self.purpose in ["Material Issue", "Material Transfer", "Manufacture", "Repack",
				...
				if self.production_order and self.purpose == "Material Transfer for Manufacture":
					...
					if self.from_warehouse and self.pro_doc:
					...
					for item in item_dict.values():
						...
						item["from_warehouse"] = self.from_warehouse if self.purpose=="Subcontract" else ""
					...
					for item in scrap_item_dict.values():
						if self.pro_doc and self.pro_doc.scrap_warehouse:
							item["from_warehouse"] = self.pro_doc.scrap_warehouse
	...
	def load_items_from_bom(self):
		if self.production_order:
			...
			from_warehouse = self.pro_doc.fg_warehouse
		else:
		...
		if not self.production_order and not from_warehouse:
			# in case of BOM
	...
	def get_bom_raw_materials(self, qty):
		...
		for item in item_dict.values():
			item.to_warehouse = self.to_warehouse or item.default_warehouse
		return item_dict
	...
	def get_transfered_raw_materials(self):
		transferred_materials = frappe.db.sql("""
			...
			from `tabOPM Stock Entry` se,`tabOPM Stock Entry Detail` sed
			where
		...
		materials_already_backflushed = frappe.db.sql("""
			...
			from
				`tabOPM Stock Entry` se, `tabOPM Stock Entry Detail` sed
			where
	...
	def get_issued_qty(self):
		...
		result = frappe.db.sql("""select t1.item_code, sum(t1.qty)
			from `tabOPM Stock Entry Detail` t1, `tabOPM Stock Entry` t2
