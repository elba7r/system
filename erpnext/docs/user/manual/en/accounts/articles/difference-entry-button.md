...
As per accounting standards, debit in a accounting entry must be equal to credit. If not, system does allow submission of accounting transaction, thereby stops ledger posting. In Revalue, on saving accounting entry, system validates if debit and credit is tallying.
